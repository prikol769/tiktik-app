        ...
        <div className="border-t-2 border-gray-200 pt-4 px-10 bg-[#F8F8F8] border-b-2 lg:pb-0 pb-[100px]">
            <div className="overflow-scroll lg:h-[450px]">
                {comments?.length ? (
                            ...
                            {allUsers.map((user: IUser) => (
                                user._id === (comment.postedBy._id || comment.postedBy._ref) && (
                                    <div className="p-2 items-center" key={index}>
                                        ...
                                        <div>
                                            <p className="px-11 py-4">{comment.comment}</p>
                                        </div>
                                    </div>
                                )
                            ))}
                    ...
                    <NoResults type="comment" text="No comments yet!"/>
                )}
            </div>

            {userProfile && (
                <div className="absolute bottom-0 left-0 right-0 pb-6 px-10">
                    <form onSubmit={addComment} className="flex gap-4">
                        <input
                            value={comment}
                            onChange={(e) => setComment(e.target.value)}
                            placeholder="Add comment..."
                            className="bg-primary px-6 py-4 text-medium
                            font-medium border-2 w-[70%]
